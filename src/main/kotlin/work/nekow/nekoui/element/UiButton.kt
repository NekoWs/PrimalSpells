...
class UiButton(
    ...
    override fun render(screen: UiScreen?, graphics: GuiGraphicsExtractor, mouseX: Int, mouseY: Int, partialTick: Float) {
        ...
        val bg = when {
            ...
            else -> cssColorState("background", background, hoverBackground, mouseX, mouseY)
        }
        ...
        val color = when {
            ...
            else -> cssColorState("color", textColor, hoverTextColor, mouseX, mouseY)
        }
        val lineH = (font.lineHeight * fontScale).toInt()
        // 原版按钮语义：(height - 8) / 2（8 = lineHeight - 1），文字视觉居中与 label 盒中心对齐
        val ty = y + (height - lineH + 1) / 2
        if (fontScale == 1f) {
            when (align) {
