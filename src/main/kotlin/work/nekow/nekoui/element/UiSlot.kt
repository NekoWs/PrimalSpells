...
class UiSlot(
    ...
    fun drawBackground(graphics: GuiGraphicsExtractor, mouseX: Int, mouseY: Int) {
        ...
        // 描边（CSS border-color 规则可覆盖，:hover 状态用 hoverBorderColor）
        val hovered = contains(mouseX.toDouble(), mouseY.toDouble())
        val border = if (hovered && hoverHighlight) cssColorState("border-color", hoverBorderColor, hoverBorderColor, mouseX, mouseY)
        else cssColorState("border-color", borderColor, borderColor, mouseX, mouseY)
